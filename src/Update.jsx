...
export default function FormComponent() {
      const { user, isAuthenticated, updateHometel } = useAuth();
  ...
  useEffect(() => {
    ...
    // setLoading(true);
    axios
      .get(`${ServerUrl}/listing/update/${id}`, {
      ...
      .then((response) => {
        if (isMounted && response.status === 200) {
          ...
          // Ensure that response.data.category is an array
          const categories = Array.isArray(response.data.category)
            ? response.data.category // If it's already an array
            : response.data.category.split(","); // If it's a string, split it into an array

          setSelectedOptions((prev) => [...prev, ...categories]); // Append categories correctly
          setloaDing(false);
        }
      })
      .catch((err) => {
    ...
    return () => {
      isMounted = false; // Cleanup and set isMounted to false when the component unmounts
    };
  }, [id, navigate]);
  useEffect(() => {
    if (
      listingDetails.country !== null &&
      listingDetails.state !== null &&
      listingDetails.city !== null
    ) {
      // get state list 
       let StatesData = state.find(
         (state) => state.id == listingDetails.country.id
       );
       setStatesList(StatesData.states);
          ...
          let CityData = StateData.states.find(
            (state) => state.id == listingDetails.state.id
          );
  ...
  const handleSubmit = async (event) => {
    ...
    if ("__v" in listingDetails) {
      delete listingDetails["__v"];
    }
    ...
    try {
      ...
      formValid = false; // ❌ Mark form invalid if any errors occur
    ...
    // ✅ Prevent submission if form is still invalid
    if (!formValid) {
    ...
    // ✅ Form is valid, proceed with submission
      ...
      let details = { ...listingDetails, category: selectedOptions };
      const data = await updateHometel({ listing: details, image: selectedFile }, id);
      if (!data.success) { 
        setLoading(false);
  ...
  const borderColor = error
    ...
    ? "border-green-500"
    : "border-gray-400";
  // Render the form or the card once the data is available
  if (loaDing === true) {
    return <LoaDer/>;
  }
  return (
          ...
          <div className="relative">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {selectedOptions.map((option, index) => (
                <div
                  key={index} // Use a unique key, here index is used as fallback
